<template>
  <footer class="footer">
    <div class="footer__links-box wrap">
      <div class="footer__links us">
        <h4 class="links-title">bilibili</h4>
        <ul class="links">
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/aboutUs.html">关于我们</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/contact.html">联系我们</a></li>
          <li class="link-item"><a target="_blank" href="https://www.bilibili.com/protocal/licence.html">用户协议</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/join.html">加入我们</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/friends-links.html">友情链接</a></li>
          <li class="link-item"><a target="_blank" href="https://www.bilibili.com/blackboard/privacy-pc.html">隐私政策</a></li>
          <li class="link-item"><a target="_blank" href="//account.bilibili.com/account/official/home">bilibili认证</a></li>
          <li class="link-item"><a target="_blank" href="http://ir.bilibili.com">Investor Relations</a></li>
        </ul>
      </div>
      <div class="footer__links transport">
        <h4 class="links-title">传送门</h4>
        <ul class="links">
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/help.html">帮助中心</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/video/BV1Xx411c7cH/">高级弹幕</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/topic_list.html">活动专题页</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/v/copyright/intro/">侵权申诉</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/x/act_list/">活动中心</a></li>
          <li class="link-item"><a target="_blank" href="https://t.bilibili.com/topic/name/%E7%94%A8%E6%88%B7%E5%8F%8D%E9%A6%88%E8%AE%BA%E5%9D%9B/feed">用户反馈论坛</a></li>
          <li class="link-item"><a target="_blank" href="//space.bilibili.com/6823116#/album">壁纸站</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/basc">专车号服务中心</a></li>
          <li class="link-item"><a target="_blank" href="//www.bilibili.com/blackboard/activity-S1jfy69Jz.html">名人堂</a></li>
        </ul>
      </div>
      <div class="medias">
        <a href="//app.bilibili.com/" target="_blank" class="media-item biliapp">
          <i class="bili-footer-font bili-footer-icon_download"></i>
          <p>客户端下载</p>
          <div class="qrcode"></div>
        </a>
        <a href="//weibo.com/bilibiliweb" target="_blank" class="media-item weibo">
          <i class="bili-footer-font bili-footer-icon_weibo"></i>
          <p>新浪微博</p>
          <div class="qrcode"></div>
        </a>
        <a class="media-item wechat">
          <i class="bili-footer-font bili-footer-icon_wechat"></i>
          <p>官方微信</p>
          <div class="qrcode"></div>
        </a>
      </div>
    </div>
    <div class="footer__partners wrap">
      <div class="authorities">
        <img src="/cdn/https://s1.hdslb.com/bfs/seed/jinkela/footer-v2/images/partner.png" width="96" height="75" class="pic">
        <img src="/cdn/https://s1.hdslb.com/bfs/static/jinkela/long/images/pic962110.png" width="100" height="40" class="pic">
        <a id="jvs-cert" href="/cdn/https://webcert.cnmstl.net/cert/grade?sn=d3ec53ae63a84350945198bab0251d59" target="_blank" style="display: block;">
          <div id="jvs-certifyOuter" class="jvs-certifyOuter">
            <img id="certify-img1" crossorigin="anonymous" src="/cdn/https://backup.hdslb.com/bfs/mainfront/websafe.png" :style="{opacity: cert}">
            <img id="certify-img2" class="act" crossorigin="anonymous" src="/cdn/https://backup.hdslb.com/bfs/mainfront/confirm.png" :style="{opacity: 1 - cert}">
          </div>
        </a>
      </div>
      <div class="notifications">
        <p>
          <span><a href="//static.hdslb.com/images/yyzz.png" target="_blank">营业执照</a></span>
          <span>信息网络传播视听节目许可证：0910417</span>
          <span>网络文化经营许可证 沪网文【2019】3804-274号</span>
          <span>广播电视节目制作经营许可证：（沪）字第01248号</span>
          <span>增值电信业务经营许可证 沪B2-20100043</span>
          <span>互联网ICP备案：<a href="http://beian.miit.gov.cn/" target="_blank">沪ICP备13002172号-3</a></span>
          <span>出版物经营许可证 沪批字第U6699 号</span>
          <span>互联网药品信息服务资格证 沪-非经营性-2016-0143</span>
          <span>营业性演出许可证 沪市文演（经）00-2253</span>
        </p>
        <p>违法不良信息举报邮箱：help@bilibili.com |违法不良信息举报电话：4000233233转3</p>
        <p>
          <i class="sprite bg1"></i>
          <a href="http://www.shjbzx.cn" target="_blank">上海互联网举报中心</a> | <a href="http://jbts.mct.gov.cn/" target="_blank">12318全国文化市场举报网站</a> | <i class="sprite bg2"></i><a href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=31011002002436" target="_blank">沪公网安备31011002002436号</a> | <a href="mailto:userreport@bilibili.com">儿童色情信息举报专区</a></p>
        <p>网上有害信息举报专区：<i class="sprite bg3"></i><a href="https://www.12377.cn/" target="_blank">中国互联网违法和不良信息举报中心</a></p>
        <p>亲爱的市民朋友，上海警方反诈劝阻电话“962110”系专门针对避免您财产被骗受损而设，请您一旦收到来电，立即接听。</p>
        <p>公司名称：上海宽娱数码科技有限公司|公司地址：上海市杨浦区政立路485号|电话：021-25099888</p>
      </div>
    </div>
  </footer>
</template>
<script>
export default {
  name: 'IndexFooter',
  data () {
    return {
      c: 0
    }
  },
  computed: {
    cert: {
      set (val) {
        this.c = val % 2
      },
      get () {
        return this.c
      }
    }
  },
  methods: {
    rolling () {
      setInterval(() => this.cert++, 3000)
    }
  },
  mounted () {
    this.rolling()
  }
}
</script>
<style lang="less" scoped>
.footer {
  padding: 30px;
  background-color: #f6f9fa;

  &__links-box {
    display: flex;
    justify-content: space-between;
    font-size: 14px;
  }

  &__links {
    padding-right: 40px;
    margin-right: 40px;

    .links-title {
      margin-bottom: 20px;
      line-height: 20px;
      color: @color-deep-gray;
      font-size: 16px;
      font-weight: normal;
    }

    .links {
      .blocking(auto, 96px, flex);
      flex-wrap: wrap;
      flex-direction: column;
    }

    .link-item {
      display: block;
      margin-bottom: 10px;
      font-size: 14px;
    }
  }

  .us,
  .transport {
    flex: 2;
    border-right: 1px solid @color-gray;
  }
  .medias {
    flex: 1;
    .blocking(auto, 136px, flex);
    .flexing(nowrap, space-between, center, center);

    .media-item {
      text-align: center;
      width: 70px;
      i.bili-footer-font {
        font-size: 40px;
      }
      p {
        margin-top: 10px;
        color: @color-text;
      }

      &.biliapp i{
        color: #585f69;
      }
      &.weibo i{
        color: #fe596c;
      }
      &.wechat i{
        color: #42c86b;
      }
    }
  }

  &__partners {
    display: flex;
    padding-top: 20px;
    justify-content: space-between;
    .authorities {
      .blocking(100px, auto, flex);
      margin-right: 10px;
      flex-wrap: nowrap;
      flex-direction: column;
      justify-content: flex-start;

      .pic {
        margin-bottom: 5px;
      }

      .jvs-certifyOuter {
        width: 100px;
        max-width: 132px;
        position: relative;

        img {
          .blocking(100%, auto);
          .p-absoluting(0, 0, 0, 0);
          transition: opacity 250ms linear;
        }
      }
    }
    .notifications {
      font-size: @font-size-small;
      line-height: @font-size-small * 2;
      color: @color-deep-gray;
      span {
        display: inline-block;
        margin-right: 15px;
      }

      .sprite {
        display: inline-block;
        width: 16px;
        height: 16px;
        vertical-align: middle;
        margin-right: 3px;
        background: url(//s1.hdslb.com/bfs/seed/jinkela/footer-v2/images/hz_icon.png) no-repeat;

        &.bg1 {
          background-position: 0 -2px;
        }

        &.bg2 {
          width: 18px;
          height: 20px;
          background-position: -41px 0;
        }

        &.bg3 {
          background-position: -18px -3px;
        }
      }
    }
  }
}
</style>
<style type="text/css" scoped>
  @font-face {
  font-family: "bili-footer-font";
  src: url(data:application/vnd.ms-fontobject;base64,);
  /* IE9 */
  src: url(data:application/vnd.ms-fontobject;base64,#iefix) format('embedded-opentype'),  url('data:application/x-font-woff2;charset=utf-8;base64,') format('woff2'), url(data:font/woff;base64,) format('woff'), url(data:font/ttf;base64,) format('truetype'),  url(//s1.hdslb.com/bfs/seed/jinkela/footer-v2/images/iconfont.svg#bili-footer-font) format('svg');
  /* iOS 4.1- */
}
.bili-footer-font {
  font-family: "bili-footer-font" !important;
  font-size: 16px;
  font-style: normal;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
}
.bili-footer-icon_weibo:before {
  content: "\E71C";
}
.bili-footer-icon_wechat:before {
  content: "\E751";
}
.bili-footer-icon_download:before {
  content: "\E752";
}
</style>
